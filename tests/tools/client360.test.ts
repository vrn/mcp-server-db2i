...
describe('getClient360Tool', () => {
  ...
  describe('successful response', () => {
    ...
    it('maps transport bloc with sites count and comparative shipping costs', async () => {
      ...
      expect(result.transport.sites_par_departement).toEqual([
        { departement: '59', quantite: 1 }
      ]);
    ...
    it('groups departments and limits to Top 10 with Autre dept fallback', async () => {
      ...
      const sites = result.transport.sites_par_departement;
      expect(sites).toHaveLength(11);
      expect(sites[0]).toEqual({ departement: '75', quantite: 3 });
      
      const findDept = (d: string) => sites.find(s => s.departement === d);
      expect(findDept('92')?.quantite).toBe(2);
      expect(findDept('93')?.quantite).toBe(2);
      expect(findDept('Autre dept')?.quantite).toBe(2);
    });
  });
